...
class DynamicRoIHeadTLD(StandardRoIHead):

    def __init__(self, **kwargs):
        super(DynamicRoIHeadTLD, self).__init__(**kwargs)
    ...
    def forward_train(self,
        ...
        if self.with_bbox or self.with_mask:
            ...
            for i in range(num_imgs):
                ...
                iou_topk = min(self.train_cfg.dynamic_tld.iou_topk,
                               len(assign_result.max_overlaps))
                for i in range(1, iou_topk):
                    ious, _ = torch.topk(assign_result.max_overlaps, i)
                    self.iou_record.append(ious[-1].item())
                cur_iou.append(mean(self.iou_record))
                self.iou_record.clear()
        ...
        update_iter_interval = self.train_cfg.dynamic_tld.update_iter_interval
    ...
    def _bbox_forward_train(self, x, sampling_results, gt_bboxes, gt_labels,
        ...
        #beta_topk = min(self.train_cfg.dynamic_tld.beta_topk * num_imgs,
          #              num_pos)
        beta_topk = min(self.train_cfg.dynamic_tld.beta_topk, num_pos)
        for i in range(1, beta_topk):
            cur = torch.kthvalue(cur_target, i)[0].item()
            self.beta_record.append(cur)
        self.beta_history.append(mean(self.beta_record))
        self.beta_record.clear()
    ...
    def update_hyperparameters(self):
        ...
        """
        new_iou_thr = max(self.train_cfg.dynamic_tld.initial_iou,
        ...
        if (np.median(self.beta_history) < EPS):
            ...
            new_beta = min(self.train_cfg.dynamic_tld.initial_beta,
